...
import { ArrowLeft, Check, ImagePlus, Trash2, Upload, X } from "lucide-react";
...
export default function AdminUploader() {
  ...
  const removePhoto = (id: string) => { const next = saved.filter(item => item.id !== id); localStorage.setItem("eunsu-upload-records", JSON.stringify(next)); setSaved(next); };
